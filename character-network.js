...
(function(){
  ...
  const graphView={scale:1,x:0,y:0,autoFit:true,expanded:false,showLabels:false,neighborhood:false,selected:'',selectedRelation:'',series:'',layoutKey:'',layout:null,data:null,pointers:new Map(),gesture:null,dragged:false,dragDistance:0,suppressClickUntil:0,pendingInitial:false,resizeObserver:null};
  ...
  function ensureUi(){
    ...
    el('cnSeriesSelect').addEventListener('change',()=>{activeSeries=el('cnSeriesSelect').value;graphView.selected='';graphView.selectedRelation='';graphView.neighborhood=false;el('cnNeighborhood').checked=false;graphView.autoFit=true;render()});
  ...
  function graphLayoutKey(chars,rels){
    return activeSeries+'|'+chars.map(c=>c.id).sort().join('\u0001')+'|'+rels.map(r=>[r.from,r.to].sort().join('\u0001')).sort().join('\u0002');
  }
  ...
  function graphEdgeGeometry(a,b,offset){
    ...
    const t=Math.min((CharacterGraphLayout.NODE_W/2+4)/Math.max(1,Math.abs(dx)),(CharacterGraphLayout.NODE_H/2+4)/Math.max(1,Math.abs(dy)),.43);
  ...
  function graphRenderInspector(){
    ...
    if(!c){box.innerHTML='<div class="cn-inspector-main"><div class="cn-kicker">NETZANSICHT</div><p>Wähle eine Figur oder Verbindung. Du kannst das Netz frei verschieben, zoomen oder auf das direkte Umfeld einer Figur reduzieren.</p></div>';return}
    const rels=d.allRels.filter(r=>r.from===c.id||r.to===c.id);
    box.innerHTML=`<div class="cn-inspector-main"><div class="cn-kicker">AUSGEWÄHLTE FIGUR</div><h4>${safeEsc(c.name)}</h4><p>${safeEsc([c.role,c.faction,statusLabel(c.status)].filter(Boolean).join(' · ')||'Figur')} · ${rels.length} Verbindung${rels.length===1?'':'en'}</p>${c.spoiler?'<span class="cn-relation-chip">Spoiler markiert</span>':''}</div><button type="button" class="cn-mini" data-cn-edit-selected="${safeEsc(c.id)}">Figur bearbeiten</button>`;
  }
  ...
  function renderCanvas(chars,rels){
    ...
    const g=graphView;if(g.series!==activeSeries){g.series=activeSeries;g.selected='';g.selectedRelation='';g.neighborhood=false;g.layoutKey='';g.autoFit=true;el('cnNeighborhood').checked=false}
